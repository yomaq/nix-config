...
op read op:"//nix/$hostname-initrd/private key?ssh-format=openssh" > "$temp/etc/ssh/$hostname.key"
op read op:"//nix/$hostname-initrd/public key" > "$temp/etc/ssh/$hostname.pub"

# Set the correct permissions so sshd will accept the key
chmod 600 "$temp/etc/ssh/$hostname"
chmod 600 "$temp/etc/ssh/$hostname.key"
